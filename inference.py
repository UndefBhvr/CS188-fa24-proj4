...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(
        ...
        "*** YOUR CODE HERE ***"
        curFacLs = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for elimVar in eliminationOrder:
            curFacLs, joinedFac = joinFactorsByVariable(curFacLs, elimVar)
            if len(joinedFac.unconditionedVariables()) > 1:
                elimFac = eliminate(joinedFac, elimVar)
                curFacLs.append(elimFac)
            else:
                pass

        if len(curFacLs) > 0:
            fullJoint = joinFactors(curFacLs)
        else:
            return None
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key, value in self.items():
            accum += value
            accumDist.append((accum, key))

        ran = random.uniform(0, accumDist[-1][0])

        for accum, key in accumDist:
            if ran <= accum:
                return key
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghoPos in self.allPositions:
            if ghoPos == jailPos and observation is not None:
                newBeliefs[ghoPos] = 0
            else:
                pObs = self.getObservationProb(
                    observation, gameState.getPacmanPosition(), ghoPos, jailPos
                )
                newBeliefs[ghoPos] = pObs * self.beliefs[ghoPos]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for newPos, p in newPosDist.items():
                newBeliefs[newPos] += self.beliefs[pos] * p
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            distP = self.getObservationProb(
                observation,
                gameState.getPacmanPosition(),
                particle,
                self.getJailPosition(),
            )
            weights[particle] += distP
        if sum(weights.values()) == 0:
            ...
            for _ in range(len(self.particles)):
                newParticles.append(sample(weights))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            dist = self.getPositionDistribution(gameState, oldParticle)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        posLs = list(itertools.product(self.legalPositions, repeat=self.numGhosts))

        random.shuffle(posLs)

        for i in range(self.numParticles):
            self.particles.append(posLs[i % len(posLs)])
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                w *= self.getObservationProb(
                    observation[i], pacPos, particle[i], self.getJailPosition(i)
                )
            weights[particle] += w
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                dist = self.getPositionDistribution(
                    gameState, pos, i, self.ghostAgents[i]
                )
