...
def joinFactors(factors: List[Factor]):
    ...
    cv = set()
    for factor in factors:
        cv = cv.union(factor.conditionedVariables())

    cv -= uv

    f = Factor(uv, cv, factors[0].variableDomainsDict())

    for assign in f.getAllPossibleAssignmentDicts():
        p = 1
        for factor in factors:
            p *= factor.getProbability(assign)
        f.setProbability(assign, p)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        f = Factor(uv, cv, factor.variableDomainsDict())

        for assign in f.getAllPossibleAssignmentDicts():
            # Compute the sum of probabilities for each assignment
            ps = 0

            for val in factor.variableDomainsDict()[eliminationVariable]:
                m_assign = assign.copy()
                m_assign[eliminationVariable] = val
                ps += factor.getProbability(m_assign)

            f.setProbability(assign, ps)
