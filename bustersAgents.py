...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        targets = [ghoPos.argMax() for ghoPos in livingGhostPositionDistributions]
        tarDist = util.Counter()
        trueTar = targets[0]
        for target in targets:
            tarDist[target] = self.distancer.getDistance(pacmanPosition, target)
            if tarDist[trueTar] > tarDist[target]:
                trueTar = target
        actDist = util.Counter()
        act = legal[0]
        for action in legal:
            actDist[action] = self.distancer.getDistance(
                Actions.getSuccessor(pacmanPosition, action), trueTar
            )
            if actDist[act] > actDist[action]:
                act = action
        return act
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
